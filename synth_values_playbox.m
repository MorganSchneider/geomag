function [B_MF, varargout] = synth_values_playbox(r, theta, phi, m_MF, varargin)
...
% m_MF = pp;
% t = mjdTimesA;
% args = {r, theta, phi, m_MF};
% varargin = {t};
% nargs = length(args) + length(varargin);
...
if size(part,2) > 1  % extract the part relevant to the interval
    ...
    pppart = pp_cut(pp,part(1,1:2));
else                  % extract the part(1)-th polynomial piece of pp (if any)
    pppart = ppp_ce(pp,part(1));
end
end

function ppcut = pp_cut(pp,interv)
